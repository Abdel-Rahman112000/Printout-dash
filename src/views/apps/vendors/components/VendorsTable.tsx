...
import SetVendorDialog from './SetVendor'
...
const VendorsListTable = () => {
  // States
  const { vendors, handleRefreshVendorsData } = useContext(VendorsCxt)
  const [data, setData] = useState([...(vendors ?? [])])
  const [rowSelection, setRowSelection] = useState({})
  const [filteredData, setFilteredData] = useState(data ?? [])
  const [globalFilter, setGlobalFilter] = useState('')
  const [openSetVendorDialog, setOpenSetVendorDialog] = useState(false)

  useEffect(() => {
    console.log('vendorsvendors', vendors)

    if (Array.isArray(vendors)) {
      console.log('vendorsvendors22', vendors)
      setData(vendors)
  ...
  const handleDeleteVendor = async (id: number) => {
    ...
    axios
      .delete(api`dashboard/vendors/${id}`, { headers })
      .then(() => {
  ...
  const columns = useMemo<ColumnDef<User, any>[]>(
    () => [
      ...
      }),
      {
        ...
        header: 'Actions',
        cell: ({ row }) => (
          <IconButton color='error' onClick={() => handleDeleteVendor(row.original.id)}>
            <DeleteIcon />
          </IconButton>
        )
      }
  ...
  return (
    <>
      ...
      <SetVendorDialog open={openSetVendorDialog} setOpen={setOpenSetVendorDialog} />
    </>
  )
}
