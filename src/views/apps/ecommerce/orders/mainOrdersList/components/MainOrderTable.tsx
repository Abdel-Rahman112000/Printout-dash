...
import { MainOrdersCxt } from '../context/MainOrdersCxt'
...
// Column Definitions
const columnHelper = createColumnHelper<MainOrderType>()

const MainOrderListTable = () => {
  // extract data from table
  const { loadingMainOrders, mainOrderslist, refreshMainOrdersList } = useContext(MainOrdersCxt)
  // States
  ...
  const columns = useMemo<ColumnDef<MainOrderType, any>[]>(
    () => [
      {
        id: 'select',
        header: ({ table }) => (
      ...
      columnHelper.accessor('id', {
        ...
        cell: ({ row }) => (
          <Typography
            component={Link}
            href={getLocalizedUrl(`/apps/ecommerce/orders/${row.original.id}`, locale as Locale)}
            color='primary'
          ...
          <Chip
            ...
            onClick={() => {
              if (row.original?.status_show?.by === 'admin') {
                handleChangeStatus(row.original.id, row.original?.status_show?.key ?? '')
              }
            }}
          />
        )
      }),
      {
        ...
        cell: ({ row }) => (
          <Stack direction={'row'} alignItems={'center'} justifyContent={'space-around'}>
            <Avatar
              alt='payment method'
              src={row.original.method == 'cash' ? cashImg.src : MasterCardImg.src}
              sx={{ width: 24, height: 24 }}
            />
            <Typography variant='body2'>{row.original.method}</Typography>
          </Stack>
        )
      },
      {
        ...
        cell: ({ row }) => (
          <OptionMenu
            ...
            options={[{ text: 'Generate Payment Link', icon: 'tabler-copy', handleClick: async () => {} }]}
          />
        )
      }
    ],
  ...
  const table = useReactTable({
    data: (mainOrderslist ?? []) as MainOrderType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
...
export default MainOrderListTable
