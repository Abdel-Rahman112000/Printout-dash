...
const UserListTable = ({ tableData, users }: { tableData?: UsersType[]; users: User[] }) => {
  // States
  ...
  const columns = useMemo<ColumnDef<User, any>[]>(
    () => [
      ...
      columnHelper.accessor('permissions', {
        ...
        cell: ({ row }) => <Typography>{row.original.permissions?.length}</Typography>
      }),
      columnHelper.accessor('roles', {
        header: 'Roles',

        cell: ({ row }) => <Typography>{row.original.roles?.map(x => x.name)?.join(', ')}</Typography>
      })
    ],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [data, filteredData]
